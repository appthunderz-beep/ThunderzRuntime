// ThunderzEditor.cpp
// Thunderz Editor (single-file, improved, fixed)
...
// - Fixes: includes <algorithm>, use std::max<T> to avoid ambiguous max()
...
static bool LoadScene() {
    ...
    while (std::getline(fi, text)) {
        ...
        if (idpos != std::string::npos) {
            Entity e;
            // crude parse
            size_t idval = text.find(':', idpos);
            if (idval != std::string::npos) {
                size_t q1 = text.find('"', idval+1);
                size_t q2 = text.find('"', q1+1);
                if (q1!=std::string::npos && q2!=std::string::npos) {
                    e.id = text.substr(q1+1, q2-q1-1);
                }
            }
            size_t apos = text.find("\"asset\"");
            if (apos!=std::string::npos) {
                size_t val = text.find(':', apos);
                size_t q1 = text.find('"', val+1);
                size_t q2 = text.find('"', q1+1);
                if (q1!=std::string::npos && q2!=std::string::npos) {
                    e.asset = text.substr(q1+1, q2-q1-1);
                }
            }
    ...
    // left panel: assets list
    {
        ...
        for (auto &kv : g_bitmaps) {
            ...
            if (bmp) {
                ...
                float s = std::max<float>(1.0f, std::max<float>(sw, sh));
    ...
    case WM_SIZE:
        {
            ...
            g_offscreenBmp = new Bitmap(std::max<int>(1,g_clientW), std::max<int>(1,g_clientH), PixelFormat32bppARGB);
            CreateInspectorControls(hwnd);
            UpdateInspectorFields();
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int) {
    ...
    if (g_offscreenBmp) delete g_offscreenBmp;
    for (auto &kv : g_bitmaps) delete kv.second;
